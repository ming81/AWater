...
///  增加了自发光的功能。
///  对于当前项目来说，Foliage和Subsurface功能应该是不需要的，需要适时屏蔽掉。
///  屏蔽了MSA的多种材质格式选项，统一使用MSA。

Shader "Carava/MiniPBR/PBR_Metallic_Emission"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			#define _MODEL_DEFAULT //_MODEL_CLOTH _MODEL_SUBSURFACE _MODEL_SKIN _MODEL_FOLIAGE _MODEL_ANISOTROPY
		...
		Pass
		{
			...
			#pragma skip_variants DIRECTIONAL SPOT POINT_COOKIE DIRECTIONAL_COOKIE
		...
		Pass 
		{
			...
			v2f_caster vert_caster( appdata_base v )
			{
				v2f_caster o;
				UNITY_SETUP_INSTANCE_ID(v);
				UNITY_INITIALIZE_VERTEX_OUTPUT_STEREO(o);
